...
| مشاكل الأداء | 5 | 🟣 MEDIUM | ✅ #28,#29 محلولتان · ⏸️ #26,#27,#30 مؤجلات |
| مشاكل البنية | 8 | ⚫ LOW | ✅ #32,#33,#34,#35,#38 محلولتان · ⏸️ #31,#36,#37 مؤجلات |
| **المجموع** | **38** | | **24 محلولة · 13 مؤجلة · 1 تم تخطيها** |
...
### ⏸️ #30 Framer Motion على كل عنصر — مؤجلة
...
### ⏸️ #31 Prisma Schema غير مستخدم — مؤجلة
- **الملف:** `prisma/schema.prisma` — يحتوي User و Post الافتراضيين
- **سبب التأجيل:** طلب المستخدم تأجيلها

### ✅ #32 `types/report.ts` فارغ — تم الحل
- **الملف:** `types/report.ts`
- **المشكلة:** الملف فارغ تماماً — لا يحتوي على أي تعريفات أنواع
- **الحل المنفذ:** تحويل الملف إلى مركز أنواع مشترك يحتوي على واجهة `Report` الموحدة (كجزء من حل #33)

### ✅ #33 تكرار تعريف واجهة التقارير (Report Interface) — تم الحل
- **المشكلة:** واجهة `Report` مُكررة في 4 ملفات (`reports`, `delete`, `edit`, `search`) — كل واحدة 26 سطر متطابقة
- **الحل المنفذ:**
  - نقل واجهة `Report` إلى `types/report.ts` كمصدر واحد مشترك
  - استبدال التعريفات المكررة بـ `import type { Report } from "@/types/report"`
  - توفير 112 سطر من الكود المكرر

### ✅ #34 تكرار كود تحويل التاريخ الهجري — تم الحل (تلقائياً)
- **المشكلة:** نفس الدالة `convertToHijri` مُكررة في ملفين
- **الحل:** عند الفحص اتضح إن الدالة مركزية بالفعل في `lib/date-utils.ts` وكل الملفات تستخدمها عبر import — لا يوجد تكرار فعلي
...
### ⏸️ #36 الترجمة تعتمد على APIs خارجية مجانية — مؤجلة
- Google Translate API و MyMemory API (مجانية)
- MyMemory له حد 5000 حرف/يوم
- **سبب التأجيل:** تحتاج اشتراك مدفوع في خدمة ترجمة (DeepL/Google Cloud Translation)

### ⏸️ #37 Android App ID عام — مؤجلة
- **الملف:** `capacitor.config.ts`
- `appId: "com.yourcompany.reports"` - App ID تجريبي
- **سبب التأجيل:** تحتاج تحديد App ID نهائي من المستخدم
...
| 12 | توحيد تعريفات الواجهات وأنواع التقارير | ✅ تم الحل (#32, #33, #34) |
...
| مايو 2026 | #32, #33 | توحيد واجهة Report في types/report.ts + إزالة 112 سطر مكرر |
| مايو 2026 | #34 | تأكيد عدم وجود تكرار في convertToHijri (مركزية بالفعل) |
